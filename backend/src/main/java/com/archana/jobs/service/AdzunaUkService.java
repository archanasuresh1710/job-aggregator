...
public class AdzunaUkService {
    ...
    @Value("${adzuna.uk.query}")
    private String query;

    @Value("${adzuna.uk.results-per-page}")
    ...
    public List<Job> fetchJobs() {
        ...
        if ("YOUR_APP_ID".equals(appId)) {
            log.warn("Adzuna API credentials not configured. Skipping UK ingestion.");
            return jobs;
        }
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                log.error("Adzuna UK API returned HTTP {}", response.statusCode());
                return jobs;
            }
            ...
            log.info("Fetched {} jobs from Adzuna UK", jobs.size());

        } catch (Exception e) {
            log.error("Failed to fetch from Adzuna UK: {}", e.getMessage());
        }
    ...
    private Job parseJob(JsonNode node) {
        try {
            ...
            String locationName = node.path("location").path("display_name").asText("UK");
            ...
            log.warn("Failed to parse Adzuna UK job: {}", e.getMessage());
            return null;
        }
    }
}
